...
int main(){
	...
	while(cin >> s && s != "*") {//横向读 
		...
		set<string> vis;					//用来判断收件人是否重复 
		while(cin >> t && t != "*") {
			parse_address(t, user2, mta2);	//处理发件人地址
		...
		for(int i = 0; i < mta.size(); i++) {//循环次数跟需要连接的mta数目有关 
			string mta2 = mta[i];
			...
			cout << " MAIL FROM:<" << s << ">\n"; cout << "250\n";
			bool ok = false;
			for(int i = 0;i < users.size(); i++) {//循环次数跟需要连接的mta中的接收者个数有关 
